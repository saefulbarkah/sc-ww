  ...
  (CharacterBuffComponent_1 = class CharacterBuffComponent extends (
    ...
    AddAttributeRateModifierLocal(t, e, o) {
      ...
      return 0 === e
        ...
        : (((f =
            CharacterBuffController_1.default.CreateDynamicBuffRef()).StackingType = 0),
          (f.DurationPolicy = 1),
          (f.Modifiers = []),
          (f.Desc = o),
          f.Modifiers.push({
          ...
          let e =
            ...
            `(${
              this.BuffGarbageSet.has(f.Handle)
                ...
                ? '激活'
                : '失效'
            })  handle: ${f.Handle},  层数: ${f.StackCount},  等级: ${f.Level} 
    施加者: ${f.GetInstigatorActorComponent()?.Actor.GetName()},  时长: ${
              f.Duration < 0
                ? '无限'
                : f.GetRemainDuration().toFixed(1) + '/' + f.Duration.toFixed(1)
            },  ${
              0 < f.Period
                ? `周期: ${f.GetRemainPeriod()?.toFixed(1)}/` +
                  f.Period.toFixed(1)
                : ''
            }
    说明: ${f.Config?.Desc}
`;
          f.Config.GrantedTags?.forEach((t) => {
            e += `    +附加标签 ${GameplayTagUtils_1.GameplayTagUtils.GetNameByTagId(
              t
            )}
`;
          });
          for (const i of this.BuffEffectManager.GetEffectsByHandle(f.Handle))
            e += `    +激活效果 ${i.GetDebugString()}(cd:${(
              this.GetBuffEffectCd(f.Id, i.Index) /
